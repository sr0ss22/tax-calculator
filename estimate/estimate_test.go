...
func TestEstimate_ConsultationLineRejected(t *testing.T) {
	// The Design Consultation Fee was removed (RDIS-135): a "consult" line is no
	// longer a recognized kind and is rejected like any other unknown kind.
	e := newEstimator(t)
	...
	if err == nil {
		t.Errorf("Estimate() error = nil, want an error for the removed consult kind")
	}
}
